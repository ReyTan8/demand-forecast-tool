...
tab1, tab2 = st.tabs(["📈 Demand Forecast", "🔄 Conversion Scenario"])
...
with tab2:
    ...
    st.subheader("🔄 Conversion Rate Scenario")

    st.markdown("""
    Apply a conversion rate to the forecast to estimate downstream demand.
    This view shows **forecast only (no historical activity)**.
    """)

    # -------------------------
    # USER INPUT (TEXT BOX)
    # -------------------------
    conversion_input = st.text_input(
        "Conversion rate (%)",
        value="50",
        help="Enter a percentage (e.g. 25, 50, 75)"
    )
    ...
    try:
        ...
        st.warning("Please enter a valid number (e.g. 50).")
        st.stop()


    # -------------------------
    # PREP DATA - only include forecast
    # -------------------------
    
    forecast_only = forecast.tail(forecast_horizon).copy().reset_index(drop=True)

    if forecast_only.empty:
        st.warning("No forecast data available.")
        st.stop()

    forecast_only["ds"] = pd.to_datetime(forecast_only["ds"]).dt.tz_localize(None)
    forecast_only["yhat"] = pd.to_numeric(forecast_only["yhat"], errors="coerce")

    # -------------------------
    # APPLY CONVERSION
    # -------------------------
    forecast_only["converted"] = forecast_only["yhat"] * conversion_rate
    forecast_only["converted_lower"] = forecast_only["yhat_lower"] * conversion_rate
    forecast_only["converted_upper"] = forecast_only["yhat_upper"] * conversion_rate


    # -------------------------
    # COMPARISON CHART
    # -------------------------
    st.markdown("### 📊 Forecast vs Converted Demand")
    ...
    fig.add_trace(go.Scatter(
        x=forecast_only["ds"],
        y=forecast_only["converted_upper"],
        mode="lines",
        line=dict(width=0),
        showlegend=False,
        hoverinfo='skip'
    ...
    # --- Converted LOWER + fill ---
    fig.add_trace(go.Scatter(
        x=forecast_only["ds"],
        y=forecast_only["converted_lower"],
        mode="lines",
        fill="tonexty",
        fillcolor="rgba(65,182,230,0.25)",
        line=dict(width=0),
        marker=dict(size=0),
        name="Converted CI",
        hovertemplate="Lower: %{y:,.0f}"
    ))

    # --- Converted line ---
    fig.add_trace(go.Scatter(
        x=forecast_only["ds"],
        y=forecast_only["converted"],
        mode="lines",
        name=f"Converted ({conversion_input}%)",
        line=dict(color="#41B6E6", width=3, dash="dash"),
        hovertemplate="Converted: %{y:,.0f}"
    ))
    ...
    total_original = forecast_only["yhat"].sum()
    total_converted = forecast_only["converted"].sum()
    avg_converted = forecast_only["converted"].mean()

    col1, col2, col3 = st.columns(3)

    col1.metric(
        "Total forecast activity",
        f"{total_original:,.0f}"
    )

    col2.metric(
        "Total converted activity",
        f"{total_converted:,.0f}"
    )

    col3.metric(
        "Avg converted activity (monthly)",
        f"{avg_converted:,.0f}"
    )

    # -------------------------
    # REQUIREMENT METRICS (CONVERTED)
    # -------------------------
    st.markdown("### 🧮 Required capacity (converted demand)")

    # Only future forecast
    future_converted = forecast_only.copy()

    # --- Absolute (no breaches) ---
    max_required_capacity_conv = future_converted["converted_upper"].max()

    # --- Typical planning (90%) ---
    p90_required_capacity_conv = future_converted["converted"].quantile(0.90)
    ...
    c1.metric(
        "Capacity to avoid all breaches",
        f"{max_required_capacity_conv:,.0f}"
    )

    c2.metric(
        "Capacity for most months (90%)",
        f"{p90_required_capacity_conv:,.0f}"
    )
